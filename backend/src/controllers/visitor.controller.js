...
const buildVisitorFilter = (req, tabType) => {
    ...
    if (search) {
        ...
        // Strip non-digits and match phone as a number
        const digitsOnly = search.replace(/\D/g, "");
        if (digitsOnly.length >= 3) {
            // Match phone that starts with or equals the digit string
            // We convert to number for exact match (regex on Number is impossible)
...
const getSafeSort = (sortBy, sortOrder) => {
    const allowed = ["createdAt", "updatedAt", "name", "email", "status", "source", "followUpDate"];
...
const sendStudentWelcomeEmail = async ({ to, name, password, courseTitle, paid, remaining }) => {
    ...
    const transporter = nodemailer.createTransport({ service: "gmail", auth: { user: process.env.EMAILID, pass: process.env.PASSKEY } });
...
exports.createVisitor = async (req, res) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(course)) return res.status(400).json({ message: "Invalid course ID" });
        ...
        if (email) {
            ...
            if (existing) return res.status(400).json({ message: "Visitor with this email already exists" });
        }

        const visitor = await Visitor.create({ ...req.body, status: req.body.status || "new", isDeleted: false, deletedAt: null });
...
exports.getNotInterestedVisitors = (req, res) => listVisitorsByTab(req, res, "not-interested");
exports.getFollowUpVisitors = (req, res) => listVisitorsByTab(req, res, "follow-up");
exports.getConvertedVisitors = (req, res) => listVisitorsByTab(req, res, "converted");
...
exports.updateVisitor = async (req, res) => {
    try {
        ...
        if (req.body.course) {
            if (!mongoose.Types.ObjectId.isValid(req.body.course)) return res.status(400).json({ message: "Invalid course ID" });
...
exports.convertToStudent = async (req, res) => {
    ...
    try {
        ...
        if (!visitor) { await session.abortTransaction(); return res.status(404).json({ message: "Visitor not found" }); }
        if (visitor.status === "converted") { await session.abortTransaction(); return res.status(400).json({ message: "Visitor already converted" }); }
        if (!visitor.email) { await session.abortTransaction(); return res.status(400).json({ message: "Email is required for conversion" }); }
        if (!visitor.course) { await session.abortTransaction(); return res.status(400).json({ message: "Visitor must have a course assigned" }); }

        const existingStudent = await Student.findOne({ visitor: visitor._id, isDeleted: false }).session(session);
        if (existingStudent) { await session.abortTransaction(); return res.status(400).json({ message: "Student already exists for this visitor" }); }

        const existingUser = await User.findOne({ email: visitor.email, isDeleted: false }).session(session);
        if (existingUser) { await session.abortTransaction(); return res.status(400).json({ message: "A user account already exists with this email" }); }

        const courseDoc = await Course.findOne({ _id: visitor.course, isDeleted: false }).session(session);
        if (!courseDoc) { await session.abortTransaction(); return res.status(400).json({ message: "Course not found. Update visitor course first." }); }
        if (!courseDoc.price || courseDoc.price <= 0) { await session.abortTransaction(); return res.status(400).json({ message: "Course price is missing. Set the course price first." }); }

        courseTitle = courseDoc.title;
        studentEmail = visitor.email;

        const { adhaar, address, dateOfBirth, gender, guardianName, guardianPhone, status, profileImage, identityProof, documents } = req.body;
        ...
        if (dateOfBirth && isNaN(dobValue?.getTime())) { await session.abortTransaction(); return res.status(400).json({ message: "Invalid dateOfBirth" }); }

        const student = await Student.create([{
            visitor: visitor._id, enrollmentDate: new Date(), status: studentStatus,
            adhaar: adhaar || undefined, address: address || undefined, dateOfBirth: dobValue || undefined,
            gender: studentGender, guardianName: guardianName || undefined, guardianPhone: guardianPhone || undefined,
            profileImage: profileImage?.url ? { url: profileImage.url, publicId: profileImage.publicId || undefined } : undefined,
            identityProof: identityProof?.type || identityProof?.number || identityProof?.frontImage?.url || identityProof?.backImage?.url
                ? { type: identityProof?.type || undefined, number: identityProof?.number || undefined, frontImage: identityProof?.frontImage?.url ? { url: identityProof.frontImage.url, publicId: identityProof.frontImage.publicId || undefined } : undefined, backImage: identityProof?.backImage?.url ? { url: identityProof.backImage.url, publicId: identityProof.backImage.publicId || undefined } : undefined }
                : undefined,
            documents: Array.isArray(documents) ? documents : [], isActive: true, isDeleted: false,
        }], { session });

        const studentDoc = student[0];
        studentPassword = generateRandomPassword(10);
        const user = await User.create([{ email: visitor.email, password: studentPassword, role: "student", student: studentDoc._id }], { session });

        const { paymentType = "full", paymentMode = "offline", amountPaid, dueDate, note, batch } = req.body;
        let batchDoc = null;
        if (batch) {
            batchDoc = await Batch.findOne({ _id: batch, isDeleted: false }).session(session);
            if (!batchDoc) { await session.abortTransaction(); return res.status(404).json({ message: "Batch not found" }); }
        }
        ...
        if (paymentType === "full") {
            ...
            if (!amountPaid || isNaN(paidAmount) || paidAmount < 0) { await session.abortTransaction(); return res.status(400).json({ message: "Valid amountPaid is required for partial payment" }); }
        }
        ...
        if (dueDate && isNaN(dueDateValue?.getTime())) { await session.abortTransaction(); return res.status(400).json({ message: "Invalid dueDate" }); }

        const fees = await Fees.create([{
            student: studentDoc._id, course: courseDoc._id, batch: batchDoc?._id || null,
            coursePrice, paymentType, paymentMode, amountPaid: paidAmount, remainingAmount,
            status: feesStatus, dueDate: dueDateValue, note: note || "",
        }], { session });
        ...
        sendStudentWelcomeEmail({ to: studentEmail, name: visitor.name, password: studentPassword, courseTitle, paid: paidAmount, remaining: remainingAmount })
            .catch((e) => console.error("Student welcome email failed:", e));

        return res.status(201).json({ message: "Visitor converted to student successfully", visitor, student: studentDoc, user: { email: user[0].email, role: user[0].role }, fees: fees[0] });
    } catch (err) {
...
exports.convertToEmployee = async (req, res) => {
    ...
    try {
        ...
        if (visitor.status === "converted") { await session.abortTransaction(); return res.status(400).json({ message: "Visitor already converted" }); }
        if (!visitor.email) { await session.abortTransaction(); return res.status(400).json({ message: "Email is required for conversion" }); }
        if (!department || !designation) { await session.abortTransaction(); return res.status(400).json({ message: "Department and designation are required" }); }

        const existingUser = await User.findOne({ email: visitor.email, isDeleted: false }).session(session);
        if (existingUser) { await session.abortTransaction(); return res.status(400).json({ message: "A user account already exists with this email" }); }

        const employee = await Employee.create([{
            name: visitor.name, email: visitor.email, phone: visitor.phone || undefined,
            department, designation, salary: Number(salary || 0), isActive: true, isDeleted: false,
        }], { session });
        ...
        const user = await User.create([{ email: visitor.email, password: defaultPassword, role, employee: employeeDoc._id }], { session });
        ...
        sendEmployeeWelcomeEmail({ to: visitor.email, name: visitor.name, password: defaultPassword, designation, department })
            .catch((e) => console.error("Employee welcome email failed:", e));

        return res.status(201).json({ message: "Visitor converted to employee successfully", visitor, employee: employeeDoc, user: { email: user[0].email, role: user[0].role } });
    } catch (err) {
        await session.abortTransaction();
