...
const EditVisitorModal = ({ open, onClose, visitor, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ parent will close modal + refresh
      onSuccess?.();
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="grid grid-cols-1 gap-4">
          <div>
            ...
            <input
              ...
              required
              className="mt-2 w-full px-3 py-2.5 rounded-2xl border border-[#DBE2EF] dark:border-slate-700 bg-white/70 dark:bg-slate-800/70 dark:text-[#DBE2EF] outline-none focus:ring-2 focus:ring-[#3F72AF]/40"
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                value={form.source}
                onChange={handleChange}
                className="mt-2 w-full px-3 py-2.5 rounded-2xl border border-[#DBE2EF] dark:border-slate-700 bg-white/70 dark:bg-slate-800/70 dark:text-[#DBE2EF]"
              >
        ...
        <div className="pt-2 flex flex-col-reverse md:flex-row md:items-center md:justify-between gap-3">
          <button
            ...
            onClick={onClose}
            disabled={loading}
            className="w-full md:w-auto px-4 py-2.5 rounded-2xl border border-[#DBE2EF] dark:border-slate-700 text-sm font-medium text-[#112D4E] dark:text-[#DBE2EF] hover:bg-[#DBE2EF]/60 dark:hover:bg-slate-800 transition disabled:opacity-60"
          >
            Cancel
          </button>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full md:w-auto px-5 py-2.5 rounded-2xl bg-[#3F72AF] hover:bg-[#2f5d95] text-white text-sm font-semibold transition disabled:opacity-60 flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
                <Loader2 size={16} className="animate-spin" />
